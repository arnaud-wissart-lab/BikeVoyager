#!/usr/bin/env sh
set -eu

valhalla_dir="${VALHALLA_DATA_DIR:-/custom_files}"
data_dir="$valhalla_dir/data"
active_dir="$valhalla_dir/live"
releases_dir="$valhalla_dir/releases"
legacy_tiles_dir="$valhalla_dir/tiles"
legacy_config_path="$valhalla_dir/valhalla.json"
legacy_admins_path="$valhalla_dir/admins.sqlite"
legacy_timezones_path="$valhalla_dir/timezones.sqlite"
pbf_path="$data_dir/osm.pbf"
logs_dir="$valhalla_dir/logs"
status_path="$valhalla_dir/build-status.json"
update_status_path="$valhalla_dir/update-status.json"
update_marker_path="$valhalla_dir/.valhalla_update_available"
build_lock_path="$valhalla_dir/.build.lock"
france_url="${VALHALLA_SOURCE_URL:-https://download.geofabrik.de/europe/france-latest.osm.pbf}"
stale_lock_minutes="${VALHALLA_STALE_LOCK_MINUTES:-30}"

mkdir -p "$data_dir"
mkdir -p "$logs_dir"
mkdir -p "$releases_dir"

is_truthy() {
  value="$(printf '%s' "${1:-}" | tr '[:upper:]' '[:lower:]')"
  [ "$value" = "1" ] || [ "$value" = "true" ] || [ "$value" = "yes" ]
}

now_utc() {
  date -u +"%Y-%m-%dT%H:%M:%SZ"
}

escape_json() {
  printf '%s' "${1:-}" | sed 's/\\/\\\\/g; s/"/\\"/g'
}

write_status() {
  state="$1"
  phase="$2"
  progress="$3"
  message="$4"
  updated_at="$(now_utc)"

  echo "[valhalla-bootstrap][build:${progress}%] ${phase} - ${message}"
  cat > "$status_path" <<EOF
{"state":"$state","phase":"$phase","progress_pct":$progress,"message":"$(escape_json "$message")","updated_at":"$updated_at"}
EOF
}

write_update_status() {
  state="$1"
  update_available="$2"
  reason="$3"
  message="$4"
  checked_at="$(now_utc)"

  cat > "$update_status_path" <<EOF
{"state":"$state","update_available":$update_available,"reason":"$(escape_json "$reason")","message":"$(escape_json "$message")","checked_at":"$checked_at","next_check_at":null,"remote":{"etag":null,"last_modified":null,"content_length":null,"checked_at":"$checked_at","available":false,"error":null}}
EOF
}

file_size_or_zero() {
  file="$1"
  if [ ! -f "$file" ]; then
    echo "0"
    return
  fi

  wc -c < "$file" | tr -d ' '
}

artifact_reason="ok"
validate_artifacts_dir() {
  base_dir="$1"
  tiles_dir="$base_dir/tiles"
  config_path="$base_dir/valhalla.json"
  admins_path="$base_dir/admins.sqlite"
  config_size="$(file_size_or_zero "$config_path")"
  admins_size="$(file_size_or_zero "$admins_path")"

  artifact_reason="ok"

  if [ ! -d "$tiles_dir" ]; then
    artifact_reason="dossier des tuiles absent"
    return 1
  fi

  if [ "$config_size" -lt 100 ]; then
    artifact_reason="fichier valhalla.json absent ou vide"
    return 1
  fi

  if [ "$admins_size" -lt 1024 ]; then
    artifact_reason="fichier admins.sqlite absent ou trop petit"
    return 1
  fi

  if ! find "$tiles_dir" -name '*.gph' -type f -print -quit 2>/dev/null | grep -q .; then
    artifact_reason="aucune tuile .gph detectee"
    return 1
  fi

  return 0
}

migrate_legacy_to_live() {
  if validate_artifacts_dir "$active_dir"; then
    return 0
  fi

  if ! validate_artifacts_dir "$valhalla_dir"; then
    return 0
  fi

  echo "Valhalla bootstrap: migration des artefacts legacy vers live."
  mkdir -p "$active_dir"

  if [ -e "$legacy_tiles_dir" ]; then
    rm -rf "$active_dir/tiles"
    mv "$legacy_tiles_dir" "$active_dir/tiles"
  fi

  if [ -f "$legacy_config_path" ]; then
    rm -f "$active_dir/valhalla.json"
    mv "$legacy_config_path" "$active_dir/valhalla.json"
  fi

  if [ -f "$legacy_admins_path" ]; then
    rm -f "$active_dir/admins.sqlite"
    mv "$legacy_admins_path" "$active_dir/admins.sqlite"
  fi

  if [ -f "$legacy_timezones_path" ]; then
    rm -f "$active_dir/timezones.sqlite"
    mv "$legacy_timezones_path" "$active_dir/timezones.sqlite"
  fi
}

run_step() {
  phase="$1"
  progress="$2"
  status_message="$3"
  label="$4"
  log_prefix="$5"
  command="$6"

  stdout_log="$logs_dir/${log_prefix}.stdout.log"
  stderr_log="$logs_dir/${log_prefix}.stderr.log"
  rm -f "$stdout_log" "$stderr_log"

  write_status "running" "$phase" "$progress" "$status_message"
  echo "Valhalla bootstrap: $label (logs: $stdout_log, $stderr_log)."

  sh -c "$command" >"$stdout_log" 2>"$stderr_log" &
  pid=$!
  started_at="$(date +%s)"
  next_heartbeat=$((started_at + 30))

  while kill -0 "$pid" 2>/dev/null; do
    now="$(date +%s)"
    if [ "$now" -ge "$next_heartbeat" ]; then
      elapsed_minutes=$(((now - started_at) / 60))
      bytes_out="$(file_size_or_zero "$stdout_log")"
      bytes_err="$(file_size_or_zero "$stderr_log")"
      bytes_total=$((bytes_out + bytes_err))
      megabytes="$(awk "BEGIN { printf \"%.2f\", $bytes_total / 1048576 }")"
      write_status "running" "$phase" "$progress" "$status_message En cours (${elapsed_minutes} min, logs ${megabytes} Mo)."
      next_heartbeat=$((now + 30))
    fi
    sleep 5
  done

  if ! wait "$pid"; then
    echo "Valhalla bootstrap: echec de l'etape '$label'."
    if [ -f "$stdout_log" ]; then
      echo "----- $label stdout (40 dernieres lignes) -----"
      tail -n 40 "$stdout_log"
    fi
    if [ -f "$stderr_log" ]; then
      echo "----- $label stderr (40 dernieres lignes) -----"
      tail -n 40 "$stderr_log"
    fi
    return 1
  fi

  return 0
}

release_stale_lock_if_needed() {
  if [ ! -f "$build_lock_path" ]; then
    return 0
  fi

  case "$stale_lock_minutes" in
    ''|*[!0-9]*) stale_lock_minutes=30 ;;
  esac
  if [ "$stale_lock_minutes" -lt 5 ]; then
    stale_lock_minutes=5
  fi

  if find "$build_lock_path" -mmin +"$stale_lock_minutes" -print -quit 2>/dev/null | grep -q .; then
    rm -f "$build_lock_path"
    echo "Valhalla bootstrap: verrou stale supprime."
    return 0
  fi

  return 1
}

download_osm_source() {
  tmp_path="${pbf_path}.tmp"
  rm -f "$tmp_path"

  if command -v curl >/dev/null 2>&1; then
    curl -fsSL "$france_url" -o "$tmp_path"
  elif command -v wget >/dev/null 2>&1; then
    wget -O "$tmp_path" "$france_url"
  else
    echo "Valhalla bootstrap: curl ou wget requis pour telecharger l'extract OSM."
    return 1
  fi

  mv "$tmp_path" "$pbf_path"
  return 0
}

finalize_failure() {
  write_status "failed" "error" "0" "Echec du build Valhalla."
  write_update_status "error" "true" "build_failed" "Echec du build Valhalla."
}

cleanup_on_exit() {
  code="$?"
  rm -f "$build_lock_path"

  if [ "$code" -ne 0 ]; then
    finalize_failure
  fi

  exit "$code"
}

write_status "running" "initialisation" "0" "Initialisation du bootstrap Valhalla."

if [ -f "$build_lock_path" ] && ! release_stale_lock_if_needed; then
  write_status "running" "initialisation" "2" "Build deja en cours sur le volume partage."
  echo "Valhalla bootstrap: build deja en cours, sortie sans erreur."
  exit 0
fi

echo "$(now_utc)" > "$build_lock_path"
trap cleanup_on_exit EXIT

force_rebuild_value="${VALHALLA_FORCE_REBUILD:-}"
force_download_value="${VALHALLA_FORCE_DOWNLOAD:-}"

migrate_legacy_to_live

already_ready=false
if validate_artifacts_dir "$active_dir" && [ -f "$active_dir/tiles/.valhalla_ready" ] && [ -f "$pbf_path" ]; then
  already_ready=true
fi

if [ "$already_ready" = true ] && ! is_truthy "$force_rebuild_value" && ! is_truthy "$force_download_value"; then
  echo "Valhalla bootstrap: donnees deja pretes, aucun rebuild."
  write_status "completed" "ready" "100" "Donnees Valhalla deja pretes."
  write_update_status "up_to_date" "false" "ok" "Donnees OSM deja a jour."
  rm -f "$update_marker_path"
  exit 0
fi

if [ ! -f "$pbf_path" ] || is_truthy "$force_download_value"; then
  reason="fichier absent"
  if is_truthy "$force_download_value"; then
    reason="force download"
  fi

  write_status "running" "download" "10" "Telechargement du fichier OSM France (${reason})."
  echo "Valhalla bootstrap: telechargement de france-latest.osm.pbf (plusieurs Go)."
  download_osm_source
else
  echo "Valhalla bootstrap: OSM France deja present, pas de telechargement."
fi

write_status "running" "preparation" "20" "Preparation des dossiers de build."

if is_truthy "$force_rebuild_value"; then
  echo "Valhalla bootstrap: rebuild force demande."
elif ! validate_artifacts_dir "$active_dir"; then
  echo "Valhalla bootstrap: rebuild requis ($artifact_reason)."
fi

release_id="$(date -u +%Y%m%dT%H%M%SZ)"
candidate_name="candidate-$release_id"
candidate_dir="$releases_dir/$candidate_name"
candidate_tiles="$candidate_dir/tiles"
candidate_ready="$candidate_tiles/.valhalla_ready"
candidate_container_dir="/custom_files/releases/$candidate_name"

rm -rf "$candidate_dir"
mkdir -p "$candidate_tiles"

echo "Valhalla bootstrap: generation des tuiles et bases dans candidate."

run_step \
  "config" \
  "30" \
  "Generation de la configuration Valhalla." \
  "generation configuration" \
  "10-config" \
  "valhalla_build_config --mjolnir-tile-dir $candidate_container_dir/tiles --mjolnir-admin $candidate_container_dir/admins.sqlite --mjolnir-timezone $candidate_container_dir/timezones.sqlite > $candidate_container_dir/valhalla.json"

run_step \
  "tiles" \
  "45" \
  "Generation des tuiles Valhalla." \
  "generation tuiles" \
  "20-tiles" \
  "valhalla_build_tiles -c $candidate_container_dir/valhalla.json /custom_files/data/osm.pbf"

run_step \
  "admins" \
  "80" \
  "Generation de admins.sqlite." \
  "generation admins" \
  "30-admins" \
  "valhalla_build_admins -c $candidate_container_dir/valhalla.json /custom_files/data/osm.pbf"

run_step \
  "timezones" \
  "90" \
  "Generation de timezones.sqlite." \
  "generation timezones" \
  "40-timezones" \
  "valhalla_build_timezones -c $candidate_container_dir/valhalla.json /custom_files/data/osm.pbf"

if ! validate_artifacts_dir "$candidate_dir"; then
  echo "Valhalla bootstrap: build termine mais artefacts invalides ($artifact_reason)."
  exit 1
fi

# Nettoyage minimal des fichiers temporaires produits pendant le build.
write_status "running" "cleanup" "95" "Nettoyage des artefacts temporaires."
find "$candidate_tiles" -name '*.tmp' -delete 2>/dev/null || true
touch "$candidate_ready"

# Promotion atomique pour conserver une bascule propre en cas d'echec.
write_status "running" "promotion" "97" "Promotion atomique des donnees Valhalla."
previous_dir="$releases_dir/previous-$release_id"

if [ -d "$active_dir" ]; then
  mv "$active_dir" "$previous_dir"
fi

if ! mv "$candidate_dir" "$active_dir"; then
  echo "Valhalla bootstrap: echec de promotion, tentative de rollback."
  if [ -d "$active_dir" ]; then
    rm -rf "$active_dir"
  fi
  if [ -d "$previous_dir" ]; then
    mv "$previous_dir" "$active_dir"
  fi
  exit 1
fi

active_config="$active_dir/valhalla.json"
if [ -f "$active_config" ]; then
  sed -i "s#/custom_files/releases/$candidate_name#/custom_files/live#g" "$active_config"
fi

rm -f "$update_marker_path"
write_update_status "up_to_date" "false" "ok" "Donnees OSM a jour apres build."
write_status "completed" "ready" "100" "Build Valhalla termine."

echo "Valhalla bootstrap: generation terminee."
echo "Release active: $active_dir"
echo "Config: $active_dir/valhalla.json"
echo "Ready: $active_dir/tiles/.valhalla_ready"
