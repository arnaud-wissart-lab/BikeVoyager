...
| `frontend/src/features/cloud/useCloudController.ts` | 687 | 586 | **RESTANT** | Encore volumineux et prioritaire pour un prochain split. |
...
| `backend/src/BikeVoyager.Infrastructure/Pois/OverpassGeometryHelper.cs` | 405 | 406 | **RESTANT** | Repasse au-dessus du seuil de 400 LOC. |
...
Verification au 2026-02-16 : 6/9 anciens hotspots sont `RESOLU`, 3/9 restent `RESTANT`.
...
Constat : 1 fichier backend depasse 400 LOC (`OverpassGeometryHelper.cs`, 406).
...
Priorisation split frontend : commencer par `frontend/src/ui/pages/DataPage.tsx`, puis `frontend/src/features/cloud/useCloudController.ts`, puis `frontend/src/features/data/useDataController.ts`.

## 3) Restant (uniquement les constats encore vrais)

- Les hotspots historiques restants sont `frontend/src/features/cloud/useCloudController.ts` (586 LOC), `frontend/src/features/routing/useRoutingController.actions.ts` (415 LOC) et `backend/src/BikeVoyager.Infrastructure/Pois/OverpassGeometryHelper.cs` (406 LOC).
- La dette de taille est desormais concentree sur le frontend (13 fichiers applicatifs > 400 LOC, +1 fichier de test > 400 LOC).
- Le backend reste majoritairement sous 400 LOC, avec 1 fichier au-dessus du seuil.
